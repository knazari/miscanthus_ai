...
class viper_robot:
    ...
    def move_to_side_pose(self, start_pose):
        ...
        side_pose.position.y = -0.2
        side_pose.position.z = 0.5
        side_quat = np.asarray(tf.transformations.quaternion_from_euler(0.0, math.pi / 10, -math.pi / 2))
    ...
    def move_to_side_down_pose(self, side_pose):
        ...
        side_down_pose.position.z -= 0.15
    ...
    def plan_and_execute_trajectory(self, waypoints, eef_step=0.01, jump_threshold=0.0, velocity_scaling=1.0):
        ...
        for i in range(len(waypoints) - 1):
            ...
            if fraction > 0.95:
                ...
                rospy.sleep(3)  # Pause for data capture
            else:
    ...
    def perform_motion_sequence(self):
        ...
        waypoints = [side_pose]
        radius = 0.3
        theta = np.linspace(-30 - 90, 30 - 90, 10, True) * (math.pi / 180.0)

        for angle in theta:
            wpose = geometry_msgs.msg.Pose()
            wpose.position.x = side_pose.position.x - radius * math.sin(angle + (math.pi / 2))
            wpose.position.y = side_pose.position.y + radius * math.cos(angle + (math.pi / 2))
            wpose.position.z = side_pose.position.z
